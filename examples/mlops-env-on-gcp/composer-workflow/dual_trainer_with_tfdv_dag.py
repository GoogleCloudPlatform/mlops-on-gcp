...
with DAG("dual_trainer_with_tfdv",
         description = "Train evaluate and validate two models on taxi fare dataset. Select the best one and register it to Mlflow v0.19",
    ...
    bq_copy = BigQueryOperator(
            ...
            sql = BQ_QUERY.format("between 0 and 5"),
            location = REGION)

    # File name must contains '*' when the output file size too large (~>240MB) e.g ds_bq_data_statistics_*.csv 
    bq_statistics_gcs_files = f"{DATASET_GCS_FOLDER}/stats_source/ds_bq_data_statistics.csv"
